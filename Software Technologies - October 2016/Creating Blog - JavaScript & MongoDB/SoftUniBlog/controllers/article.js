...
module.exports = {
    ...
    createPost: (req, res) => {
        ...
        Article.create(articleArgs).then(article => {
            //Get the tags from the input, split it by space or semicolon, then remove empty entries.
            let tagNames = articleArgs.tagNames.split(/\s+|,/).filter(tag => {
                return tag
            });
            initializeTags(tagNames, article.id);

            article.prepareToInsert();
            res.redirect('/');
        });
    },
    ...
    editPost: function (req, res) {
        ...
        if (!articleArgs.title) {
            errorMessage = 'Article title cannot be empty!';
        } else if (!articleArgs.content) {
            errorMessage = 'Article content cannot be empty!';
        }
            ...
            Article.findById(id).populate('category tags').then(article => {
                ...
                article.save((err) => {
                    ...
                    Category.findById(article.category).then(category => {
                        let categoryDoesntHaveArticle = category.articles.indexOf(article.id) === -1;
                        if (categoryDoesntHaveArticle) {
